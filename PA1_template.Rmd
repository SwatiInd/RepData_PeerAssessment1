...
```
The **mean and median** of total steps taken per day are **`r format(mean, scientific = FALSE)`** and **`r format(median, scientific = FALSE)`**, respectively.
...
```
The interval value corresponding to the maximum of average steps per interval is **`r max_interval`**.
...
```
The mean of steps per day with and without NA imputing remain the same as `r format(mean_new, scientific = FALSE)`. But, the median with and without NA imputing are different; `r format(median_new, scientific = FALSE)` and `r format(median, scientific = FALSE)`, respectively.
